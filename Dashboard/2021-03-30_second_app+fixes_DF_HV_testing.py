...
st.title('Crime in Los Angeles - 2010 to 2019')
#st.header('Interactive visual analysis of Los Angeles crimes from 2010 though 2019.')
...
left1, middle1, right1 = st.beta_columns((0.1,0.3,0.1))
...
st.subheader("Map of Los Angeles crimes in %i." % (year_selected))
st.write('')
map(crime, midpoint[0], midpoint[1], 8.5)

# with right2:
st.subheader("Top 10 most occurent crime types in %i." % (year_selected))
...
# with right3:
st.subheader(f'Top 10 most occured crime types in {area_selected} and their evolution through {year_selected}.')
...
with left4:
    ...
    st.subheader(f'Crime rate in {area_selected} compared to the Los Angeles average.')
    st.plotly_chart(bar, use_container_width= True)
    

with right4:
    
    st.subheader(f'Ethnicity distribution of the crime victims in {area_selected}.')
    st.plotly_chart(ring, use_container_width = False)
